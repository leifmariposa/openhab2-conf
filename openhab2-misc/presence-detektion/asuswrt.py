...
#            print("key:", key)
            if devices[key].ip is not None:
                ...
                print("ip:   ", devices[key].ip, "  ", devices[key].mac, "  ", devices[key].name)
        return ret_devices
...
def main():
    scanner = AsusWrtDeviceScanner("192.168.1.1", 23, "admin", "Bigoton11")
    print(scanner.success_init)
    #print(scanner.last_results)
    with open('/tmp/devices', 'w') as the_file:
        for key in scanner.last_results:
            if scanner.last_results[key].ip is not None:
#                print("scanner.last_results[key].ip, "  ", scanner.last_results[key].mac, "  ", scanner.last_results[key].name)
                the_file.write(scanner.last_results[key].ip)
                the_file.write(' ')
                the_file.write(scanner.last_results[key].mac)
                if scanner.last_results[key].name is not None:
                    the_file.write(' ')
                    the_file.write(scanner.last_results[key].name)
                the_file.write('\n')
